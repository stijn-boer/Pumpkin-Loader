...
pub struct ModDependencies {
    #[serde(default)]
    pub required: Vec<ModDependency>,
    #[serde(default)]
...
pub struct MixinDefinition {
    ...
    #[serde(default)]
    pub compatibility: MixinCompatibility,
    #[serde(default)]
    pub validation: MixinValidation,
}
...
pub struct ResolvedMods {
    pub ordered: Vec<DiscoveredMod>,
}
impl ResolvedMods {
    pub fn ids(&self) -> impl Iterator<Item = &str> {
        self.ordered
            .iter()
            .map(|item| item.manifest.mod_info.id.as_str())
    }
}
...
pub fn init(config: &Config, layout: &Layout, name: &str, force: bool) -> Result<PathBuf> {
    ...
    if mod_dir.exists() {
        if !force {
            return Err(LoaderError::ModAlreadyExists { path: mod_dir });
        }
    ...
    fs::write(
        mod_dir.join("plugin/src/lib.rs"),
        render_template(
    ...
    log::info!(
        "Created plugin/mixin mod package `{slug}` at {}",
        mod_dir.display()
    );
    Ok(mod_dir)
}
        ...
        .map_err(|source| LoaderError::io("read mods directory", &layout.mods, source))?
    {
        ...
        if manifest.format != DEFAULT_FORMAT {
            return Err(LoaderError::InvalidModManifest {
                path: manifest_path,
                message: format!(
                    "unsupported manifest format {}; expected {DEFAULT_FORMAT}",
                    manifest.format
                ),
            });
        }
...
fn validate_manifest_paths(directory: &Path, manifest: &ModManifest) -> Result<()> {
    let plugin = directory.join(&manifest.plugin.path);
    if !plugin.join("Cargo.toml").is_file() {
        return Err(LoaderError::InvalidModManifest {
            path: directory.join(MANIFEST_NAME),
            message: format!(
                "plugin path `{}` does not contain Cargo.toml",
                manifest.plugin.path.display()
            ),
        });
    }
    ...
    for mixin in &manifest.mixins {
        ...
        if !ids.insert(mixin.id.clone()) {
            return Err(LoaderError::InvalidModManifest {
                path: directory.join(MANIFEST_NAME),
                message: format!("duplicate mixin id `{}`", mixin.id),
            });
        }
        let patch = directory.join(&mixin.patch);
        if !patch.is_file() {
            return Err(LoaderError::MissingModPatch { path: patch });
        }
    }
    Ok(())
}
...
pub fn resolve_for_dev(
    ...
    let discovered = discover(config, layout, commit)?;
    let selected = if discovered.contains_key(selected) {
        selected.to_owned()
    } else {
        slugify(selected)
    };
    if !discovered.contains_key(&selected) {
        return Err(LoaderError::ModNotFound { name: selected });
    }
    resolve_from(discovered, BTreeSet::from([selected]))
}

fn resolve_from(
    discovered: HashMap<String, DiscoveredMod>,
    ...
    while let Some(id) = stack.pop() {
        ...
        let item = discovered
            .get(&id)
            .ok_or_else(|| LoaderError::MissingModDependency {
                mod_id: "root".into(),
                dependency: id.clone(),
            })?;
        for dependency in &item.manifest.dependencies.required {
            let target = discovered.get(&dependency.id).ok_or_else(|| {
                LoaderError::MissingModDependency {
                    mod_id: id.clone(),
            ...
            if !dependency
                .version
                .matches(&target.manifest.mod_info.version)
            {
                return Err(LoaderError::ModVersionMismatch {
                    mod_id: id.clone(),
                    dependency: dependency.id.clone(),
                    required: dependency.version.to_string(),
                    actual: target.manifest.mod_info.version.to_string(),
                });
            }
    ...
    for id in &selected {
        ...
        for conflict in &item.manifest.dependencies.conflicts {
            ...
            {
                return Err(LoaderError::ModConflict {
                    first: id.clone(),
                    second: conflict.id.clone(),
                });
            }
        }
    }
    ...
    let mut add_edge = |from: &str, to: &str| {
        if selected.contains(from)
            ...
            && edges
                .get_mut(from)
                .is_some_and(|set| set.insert(to.to_owned()))
        {
    ...
    for id in &selected {
        let item = &discovered[id];
        for dependency in &item.manifest.dependencies.required {
            add_edge(&dependency.id, id);
        }
        for before in &item.manifest.dependencies.load_before {
            add_edge(id, before);
        }
        for after in &item.manifest.dependencies.load_after {
            add_edge(after, id);
        }
    }
    ...
    if order.len() != selected.len() {
        return Err(LoaderError::ModDependencyCycle {
            mods: indegree
                .into_iter()
                .filter(|(_, n)| *n > 0)
                .map(|(id, _)| id)
...
pub fn prepare_worktree(
    ...
    log::info!(
        "Prepared mixins for {} mods: {}",
        mods.ordered.len(),
        mods.ids().collect::<Vec<_>>().join(", ")
    );
    Ok(worktree)
}
...
pub fn prepare_dev(config: &Config, layout: &Layout, name: &str) -> Result<PathBuf> {
    ...
    log::info!(
        "Plugin source: {}",
        selected
            .join(&load_manifest(&selected)?.plugin.path)
            .display()
    );
...
fn resolve_mixins(mods: &ResolvedMods) -> Result<Vec<ResolvedMixin<'_>>> {
    ...
    let mut edges: BTreeMap<String, BTreeSet<String>> =
        all.keys().map(|id| (id.clone(), BTreeSet::new())).collect();
    ...
    for (id, mixin) in &all {
        for after in &mixin.definition.after {
            ...
            if !all.contains_key(&dependency) {
                return Err(LoaderError::UnknownMixinReference {
                    mixin: id.clone(),
            ...
            if edges
                ...
                .insert(id.clone())
            {
                *indegree.get_mut(id).expect("mixin node") += 1;
            }
        }
        for before in &mixin.definition.before {
            ...
            if edges
                ...
                .insert(target.clone())
            {
                *indegree.get_mut(&target).expect("mixin node") += 1;
            }
        }
    }
    ...
    while let Some(id) = ready.pop_first() {
        ...
        for target in edges[&id].clone() {
            let n = indegree.get_mut(&target).expect("mixin node");
            *n -= 1;
            if *n == 0 {
                ready.insert(target);
            }
    ...
    if ordered.len() != all.len() {
        return Err(LoaderError::MixinDependencyCycle {
            mixins: indegree
                .into_iter()
                .filter(|(_, n)| *n > 0)
                .map(|(id, _)| id)
...
fn qualify_mixin_ref(owner: &str, reference: &str) -> String {
    if reference.contains(':') {
        ...
        format!("{owner}:{reference}")
    }
}

fn validate_mixin(worktree: &Path, commit: &str, mixin: &ResolvedMixin<'_>) -> Result<()> {
    let definition = mixin.definition;
    if !definition.compatibility.revisions.is_empty()
        && !definition
            .compatibility
            .revisions
            .iter()
            .any(|revision| commit == revision || commit.starts_with(revision))
    {
        return Err(LoaderError::MixinValidation {
            mixin: mixin.qualified_id.clone(),
            message: format!("commit {commit} is not in compatibility.revisions"),
        });
    }
    let package_root = if let Some(package) = &definition.target.package {
        find_package(worktree, package)?
    } else {
        worktree.to_path_buf()
    };
    let files: Vec<PathBuf> = definition
        .target
        .files
        .iter()
        .map(|file| package_root.join(file))
        .collect();
    for file in &files {
        if !file.is_file() {
            return Err(LoaderError::MixinValidation {
                mixin: mixin.qualified_id.clone(),
                message: format!("target file does not exist: {}", file.display()),
            });
        }
    }
    let combined = files
    ...
    for needle in &definition.validation.contains {
        if !combined.contains(needle) {
            return Err(LoaderError::MixinValidation {
                mixin: mixin.qualified_id.clone(),
                message: format!("required text was not found in target files: {needle:?}"),
            });
        }
    }
    for needle in &definition.validation.not_contains {
        if combined.contains(needle) {
            return Err(LoaderError::MixinValidation {
                mixin: mixin.qualified_id.clone(),
                message: format!("forbidden text is already present in target files: {needle:?}"),
            });
        }
    }
    if !definition.target.files.is_empty() {
        ...
        let output = process::output(
            Command::new("git")
                ...
                .args(["apply", "--numstat"])
                .arg(&patch),
            "inspect mixin patch paths",
        )?;
        let allowed: BTreeSet<PathBuf> = definition
            .target
            .files
            .iter()
            .map(|path| {
        ...
        for line in output.lines() {
            ...
            if !allowed.contains(&path) {
                return Err(LoaderError::MixinValidation {
                    mixin: mixin.qualified_id.clone(),
                    message: format!("patch changes undeclared target file `{}`", path.display()),
                });
            }
        }
    }
...
pub fn build_and_deploy_plugins(
    config: &Config,
    layout: &Layout,
    mods: &ResolvedMods,
) -> Result<()> {
    ...
    let expected: BTreeSet<String> = mods
        .ordered
        .iter()
        .map(|item| item.manifest.mod_info.id.clone())
        .collect();
    for entry in fs::read_dir(&plugins_dir)? {
        let entry = entry?;
        let name = entry.file_name().to_string_lossy().into_owned();
        if let Some(id) = name
            .strip_prefix("pumpkin-loader-")
    ...
    for item in &mods.ordered {
        build_and_deploy_plugin(config, layout, item, &plugins_dir)?;
    }
    Ok(())
}

fn build_and_deploy_plugin(
    config: &Config,
    layout: &Layout,
    item: &DiscoveredMod,
    plugins_dir: &Path,
) -> Result<()> {
    let plugin_root = item.directory.join(&item.manifest.plugin.path);
    let worktree = layout.worktrees.join(&item.manifest.pumpkin.revision);
    let toolchain = toolchain::resolve(&worktree)?;
    let profile = if config.build.profile == "moddev" {
        "dev"
    } else {
    ...
    let mut args = vec![
        ...
        plugin_root.join("Cargo.toml").display().to_string(),
        "--profile".to_owned(),
        profile.to_owned(),
        "--message-format=json-render-diagnostics".to_owned(),
    ];
    if let Some(package) = &item.manifest.plugin.package {
        args.extend(["--package".to_owned(), package.clone()]);
    }
    if config.build.locked && plugin_root.join("Cargo.lock").is_file() {
    ...
    envs.insert(
        ...
        layout.plugin_target.as_os_str().into(),
    );
    ...
    let output = process::output(
        &mut command,
        &format!("build plugin {}", item.manifest.mod_info.id),
    )?;
    ...
    if !artifact.is_file() {
        return Err(LoaderError::MissingPluginArtifact {
            mod_id: item.manifest.mod_info.id.clone(),
            path: artifact,
        });
    }
    let extension = artifact
        ...
        .unwrap_or("so");
    let destination = plugins_dir.join(format!(
        "pumpkin-loader-{}.{}",
        item.manifest.mod_info.id, extension
    ));
    fs::copy(&artifact, &destination)
        .map_err(|source| LoaderError::io("deploy Pumpkin plugin", &destination, source))?;
    log::info!(
        "Deployed plugin `{}` to {}",
        item.manifest.mod_info.id,
        destination.display()
    );
    Ok(())
}
...
fn find_dynamic_library_artifact(output: &str, package: Option<&str>) -> Result<PathBuf> {
    ...
    for line in output.lines() {
        ...
        if let Some(package) = package {
            ...
            if target_name.map(|name| name.replace('-', "_")) != Some(package.replace('-', "_")) {
                continue;
            }
        }
        let kinds = value
        ...
        if !kinds.is_some_and(|items| {
            items
                .iter()
                .any(|item| matches!(item.as_str(), Some("cdylib" | "dylib")))
        }) {
            continue;
        }
    ...
    candidates
        .pop()
        .ok_or(LoaderError::PluginArtifactNotReported)
}
...
pub fn create_patch(
    ...
    let worktree = layout.worktrees.join(&manifest.pumpkin.revision);
    if !worktree.join(".git").exists() {
        return Err(LoaderError::ModWorktreeNotPrepared {
    ...
    process::checked(
        ...
        "advance mixin patch baseline",
    )?;
    log::info!("Generated mixin patch {}", path.display());
    Ok(path)
}
...
pub fn hash_mixins(mods: &ResolvedMods) -> Result<Vec<u8>> {
    ...
    for item in &mods.ordered {
        hasher.update(item.manifest.mod_info.id.as_bytes());
        hasher.update([0]);
        hasher.update(item.manifest.pumpkin.revision.as_bytes());
        hasher.update([0]);
        hasher.update(serde_json::to_vec(&item.manifest.mixins)?);
        hasher.update([0]);
        for mixin in &item.manifest.mixins {
            hasher.update(fs::read(item.directory.join(&mixin.patch))?);
            hasher.update([0]);
        }
    }
...
fn find_package(worktree: &Path, package_name: &str) -> Result<PathBuf> {
    ...
    for entry in WalkDir::new(worktree).max_depth(5) {
        ...
        let document = fs::read_to_string(entry.path())?
    ...
    candidates
        ...
        .ok_or_else(|| LoaderError::InvalidModManifest {
            path: worktree.join("Cargo.toml"),
            message: format!("could not locate target package `{package_name}`"),
        })
}

fn append_mixin_to_manifest(path: &Path, id: &str, patch: &Path) -> Result<()> {
    let mut manifest: ModManifest =
        toml::from_str(&fs::read_to_string(path)?).map_err(|source| {
            LoaderError::ParseModManifest {
    ...
    if manifest.mixins.iter().any(|mixin| mixin.id == id) {
        return Err(LoaderError::InvalidModManifest {
            path: path.to_path_buf(),
            message: format!("mixin id `{id}` already exists"),
        });
    }
    manifest.mixins.push(MixinDefinition {
        ...
        after: Vec::new(),
        before: Vec::new(),
        target: MixinTarget::default(),
        compatibility: MixinCompatibility::default(),
        validation: MixinValidation::default(),
    });
    write_manifest(path, &manifest)
}

fn write_manifest(path: &Path, manifest: &ModManifest) -> Result<()> {
...
fn load_manifest(directory: &Path) -> Result<ModManifest> {
    ...
    toml::from_str(&fs::read_to_string(&path)?)
        .map_err(|source| LoaderError::ParseModManifest { path, source })
}
fn resolve_mod(layout: &Layout, name: &str) -> Result<PathBuf> {
    for candidate in [layout.mods.join(name), layout.mods.join(slugify(name))] {
...
fn ensure_revision(manifest: &ModManifest, resolved: &str, config: &Config) -> Result<()> {
    if manifest.pumpkin.revision != resolved {
        return Err(LoaderError::ModRevisionMismatch {
            mod_id: manifest.mod_info.id.clone(),
            declared: manifest.pumpkin.revision.clone(),
...
fn next_patch_number(directory: &Path) -> Result<u32> {
    ...
    for entry in fs::read_dir(directory)? {
        if let Some(prefix) = entry?.file_name().to_string_lossy().split('-').next() {
...
fn validate_name(name: &str) -> Result<()> {
    if name.trim().is_empty()
        ...
        || name == ".."
    {
        return Err(LoaderError::InvalidModName { name: name.into() });
    }
    Ok(())
}
